 ...
 * 注意:在注册没有传入auth_id 的情况下，验证时需要传入注册时返回的GID。
 */
+ (NSString*) GID;
...
/** PROPERTY
 *  属性，模型控制参数。
 *  目前支持：传入标识是否删除数据库里的用户模型。
 */
+(NSString*) PROPERTY;

#pragma mark - PROPERTY param
/** DEL
 *  属性参数值，标识删除数据库里的用户模型。
 *  注册时传入del 删除已注册的模型。
 */
...
/** UNDEL
 *  属性参数值，标识不删除删除数据库里的用户模型。
 *  注册时传入undel 不删除用户模型，继续重新注册。
 */
+(NSString*) UNDEL;

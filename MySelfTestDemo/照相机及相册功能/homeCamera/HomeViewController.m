...
- (void)viewDidLoad {
    ...
    [self.snapButton setTitle:@"确定" forState:UIControlStateNormal];
    ...
    buttonclear.frame = CGRectMake(ScreenWidth-70, ScreenHeigth-65, 50,50);
    [buttonclear setTitle:@"取消" forState:UIControlStateNormal];
    [buttonclear setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    buttonclear.layer.cornerRadius = buttonclear.width/2.0f;
    buttonclear.layer.borderColor = [UIColor whiteColor].CGColor;
    buttonclear.layer.borderWidth = 2.0f;
    buttonclear.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.5];
    buttonclear.layer.rasterizationScale = [UIScreen mainScreen].scale;
    buttonclear.layer.shouldRasterize = YES;
    [buttonclear addTarget:self action:@selector(goBackbb) forControlEvents:UIControlEventTouchUpInside];
